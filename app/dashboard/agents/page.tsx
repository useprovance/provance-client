         ...
         <div className="max-w-5xl w-full mx-auto px-8 py-10 flex flex-col gap-8">
            <div className="flex flex-col gap-3">
               ...
               <div className="flex items-center justify-between w-full border-t border-b border-sand/25 py-3">
                  <div className="flex items-center gap-2 pr-6 shrink-0">
                     ...
                     <p className="text-sand text-xs font-mono uppercase tracking-widest">My Agents</p>
                  </div>
                  <div
                     className="flex-1 h-full min-h-[16px]"
                     style={{
                        backgroundImage: "repeating-linear-gradient(-45deg, var(--sand) 0, var(--sand) 1px, transparent 0, transparent 50%)",
                  ...
                  <Link
                     ...
                     className="flex items-center gap-2 bg-sand hover:bg-sand-light text-ink-dark text-[12px] font-bold px-5 py-2 ml-6 shrink-0 transition-colors"
            ...
            <div className="relative p-1.5 border border-sand/10">
               ...
               <div className="relative z-10 grid grid-cols-2 lg:grid-cols-4 gap-1.5">
                  {[
                     { label: "Total Agents", value: MOCK_AGENTS.length, display: String(MOCK_AGENTS.length).padStart(2, "0"), icon: Bot },
                     { label: "Active Now",   value: active,             display: String(active).padStart(2, "0"),             icon: Activity },
                     { label: "Runs Today",   value: totalRuns,          display: String(totalRuns),                           icon: Zap },
                     { label: "Earned Today", value: 0,                  display: "4.89 USDC",                                 icon: TrendingUp },
                  ].map(({ label, display, icon: Icon }) => (
                     <div key={label} className="flex flex-col justify-between bg-[#1a1a1a] border border-sand/10 p-5">
